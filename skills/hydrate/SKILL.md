...
This skill keeps hydration reminders separate from the user's actual task. It uses the bundled `scripts/hydrate.py` CLI for all timing, state, and SQLite persistence.
...
```bash
python3 "$HYDRATE_CLI" drink --amount <ml> --json
```
...
```bash
python3 "$HYDRATE_CLI" status
```

For configuration changes:

```bash
python3 "$HYDRATE_CLI" config set <key> <value>
```
